...
export const FeatureDetection = {
  ...
  deviceCapabilities: {
    ...
    isLowPowerDevice: () => {
      ...
      return (
        FeatureDetection.deviceCapabilities.isOldMobile() ||
  ...
  applyFeatureClasses: () => {
    ...
    html.classList.add(
      FeatureDetection.supportsModernJS() ? "modern-js" : "legacy-js"
    );
    html.classList.add(FeatureDetection.supportsES6() ? "es6" : "no-es6");
    html.classList.add(
      FeatureDetection.supportsCSS.grid() ? "css-grid" : "no-css-grid"
    );
    html.classList.add(
      FeatureDetection.supportsCSS.flexbox() ? "flexbox" : "no-flexbox"
    );
    html.classList.add(
      FeatureDetection.supportsCSS.customProperties()
        ? "css-vars"
        : "no-css-vars"
    );
    html.classList.add(
      FeatureDetection.deviceCapabilities.supportsTouch() ? "touch" : "no-touch"
    );
    html.classList.add(
      FeatureDetection.deviceCapabilities.isLowPowerDevice()
        ? "low-power"
        : "high-power"
    );
    html.classList.add(
